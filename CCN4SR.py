...
class CCN4SR(nn.Module):
    ...
    #################### item ###########################
        # vertical conv layer
        self.conv_v2 = nn.Conv2d(1, self.n_v,( L , 1))
        # horizontal conv layer
        self.conv_h2 = nn.Conv2d(1, self.n_h, (1, dims))
    ################### item ############################
    ...
    #################### user ###########################
        ...
        self.W2 = nn.Embedding(num_items, self.fc1_dim_h + dims + self.n_u1 )   #self.fc1_dim_h + dims + self.n_u1
    ...
    ############################################################################################################
        # 垂直胶囊
        if self.n_v:
            out_v = torch.sigmoid(self.conv_v2(item_embs).squeeze(-2))
            print(out_v.shape)
            ...
            out_v = out_v.view(-1, self.fc1_dim_v)    #[512,200]
            out_v = self.dropout(out_v)
        # 水平胶囊
        if self.n_h:
            out_h = torch.sigmoid(self.conv_h2(item_embs).squeeze(-1))
            out_h = self.item_routing1(inputs=out_h, CW=self.CW1, num_capsule=5, dim_capsule=self.args.d,
                                       input_num_capsule=5, input_dim_capsule=self.args.nh, routings=3)
    ...
    ############################################################################################################
        ...
        user_merge = torch.cat([user_emb1, user_merge0], 1)
